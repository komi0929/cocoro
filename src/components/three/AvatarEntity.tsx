...
function applyIdleAnimation(
  ...
  const breathIntensity = isSpeaking ? 0.03 : 0.018;
  ...
  const weightShift = Math.sin(weightCycle) * (isSpeaking ? 0.015 : 0.02);
  ...
  if (chest) {
    ...
    const speakLean = isSpeaking ? 0.03 : 0;
  ...
  if (head) {
    ...
    const nodBase = isSpeaking ? 0.03 : 0.015;
    const targetHeadX = Math.sin(headNodCycle) * nodBase;
    const speakNod = isSpeaking ? Math.sin(time * 2.5) * 0.02 : 0;
    const targetHeadY = Math.cos(headNodCycle * 0.7) * 0.015;
  ...
  const shoulderBreath = Math.sin(breathCycle + 0.5) * (isSpeaking ? 0.012 : 0.006);
  if (leftShoulder) {
  ...
  // If speaking: lean forward slightly (set, not accumulate)
  if (isSpeaking && spine) {
    spine.rotation.x = state.spineRotX + 0.015;
  }
  ...
  if (leftLowerArm) {
    ...
    leftLowerArm.rotation.z = 0.15 + Math.sin(time * 0.4 + hash * 2) * 0.01 + gesture;
    leftLowerArm.rotation.x = isSpeaking ? Math.sin(time * 2.2) * 0.04 : 0;
  }
  if (rightLowerArm) {
    const gesture = isSpeaking ? Math.sin(time * 1.8 + hash + 2) * 0.06 : 0;
    rightLowerArm.rotation.z = -0.15 - Math.sin(time * 0.4 + hash * 2 + 0.5) * 0.01 - gesture;
